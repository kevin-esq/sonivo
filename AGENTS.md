...
**System close plan:** [`docs/01-product/SYSTEM-CLOSE-PLAN.md`](docs/01-product/SYSTEM-CLOSE-PLAN.md) — Gate A **CLOSED**. Gate B UI redesign **AUTHORIZED** ([`PHASE-GATE-B-UI-SPEC.md`](docs/03-architecture/PHASE-GATE-B-UI-SPEC.md)).  
**Next:** T-GATE-B-01–05 on `develop`. File Resource (**T-3.2.06**) remains **DEFERRED**. Do not merge to `main` until Gate B is accepted.

Until the user explicitly authorizes additional work:

- Do **not** implement file Resource (T-3.2.06)
- Do **not** expand SMTP beyond the closed thin 3.9 spec (Gmail API HTTPS only; no Event/RSVP mail, no generic SMTP server)
...
- Foundation + Group/Membership + repertoire + Phase 3.3–3.9 + Gate B UI chrome (T-GATE-B-01–05) within the accepted contracts is allowed. Do **not** merge `main` until Gate B is accepted.
